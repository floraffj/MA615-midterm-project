...
Firstly we find the combinations of different country and different year, which should be $76*25=1900$ observation. And then we select the variable "population" from raw data and matched it with the combination of country and year.
...
In order to avoid being verbose, we defined a function to select the data we need and to join them together to become a new data frame.
...
### The cereals production of Eypgt 
...
IFS_tidy %>%
  ...
  layer_lines() %>% 
  add_axis("y",title="Cereals Production") %>% 
  add_axis("x", title = "Year") %>% 
  add_axis("x", orient = "top", ticks = 0, title ="Cereals Production of Egypt")
```

A significant increasing trend can be seen from the graph above, which indicates the increase in the cereals production in Egypt throughout the period. Similar trend can be observated in Vietnam.
...
IFS_tidy %>%
  ...
  add_axis("y",title="Cereals Production") %>% 
  add_axis("x", title = "Year") %>% 
  add_axis("x", orient = "top", ticks = 0, title ="Cereals Production of Vietnam")

```

However, the cereals production had an opposite trend in Somalia.
...
IFS_tidy %>%
  ...
  add_axis("y",title="Cereals Production") %>% 
  add_axis("x", title = "Year") %>% 
  add_axis("x", orient = "top", ticks = 0, title ="Cereals Production of Somilia")
...
Interactive plot can be used to show effectively the trend of cereals production of 76 countries during the 25-year period. The interactive plot can be checked in R.
...
ggplot(aveg_ava) + 
  ...
  geom_raster()  +
  theme(axis.text.x=element_text(angle=90, hjust=1)) + ggtitle("Average Food availability")
```

Missing data will not be filled with color in the graph above. The darker blue rectangular pattern indicates a smaller average food availability in a specific year in a specific country. Egypt has much lighter rectangulat pattern than other countries, which indicates the average food availability in Egypt was larger than other countries.

## Cereals Harvested 

We use map plot to show the area harvested of cereals in each country.
...
Cereals producation per 1000 Ha can be regarded as an indictor of the cereals productivity in the countries. In the plot above, we can noticed that the cereals productivity in Egypt was significantly larger than other countries. Basically in these countries, the productivity in the recent years was higher than that in the 20th century. This increasing trend can also be verified by the graph of the average productivity throughout the period.
